        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div id="store" class="col-md-9">
                    <form action="admin_suaSP.php">
                    <div class="store-filter clearfix" style="padding-left:15px; ">
                        <input type="text" name="search">
                        <span>&nbsp</span>
                        <button type="submit" name="trang" value="1">Tìm</button> 
                    </div>
                    </form>
                    <!-- modify -->
                    ...
                    $sql = sprintf("SELECT * FROM san_pham ORDER BY id LIMIT %d, %d", $offset, $numItemPerPage) ;
                    $result = $conn->query($sql);

                    if (isset($_REQUEST['search'])){
                        $search = $_REQUEST['search'];
                        $sql_demSP = sprintf("SELECT COUNT(id) AS dem FROM san_pham WHERE ten_sp LIKE '%%%s%%'",$search);
                        $result_dem = $conn->query($sql_demSP);
                        $row_dem = $result_dem->fetch_assoc();
                        $soluong_SP = $row_dem['dem'];
                        $trang = $_GET['trang'];
                        ...
                        $so_trang = ceil($so_trang);
					    $offset = ($page - 1) * $numItemPerPage;
                        $sql = sprintf("SELECT * FROM san_pham WHERE ten_sp LIKE '%%%s%%' ORDER BY id LIMIT %d, %d", $search, $offset, $numItemPerPage) ;
                        $result = $conn->query($sql);
                    }
					...
					?>						
                        <div style="display: block" class="col-md-4 col-xs-6">
                            <div class="product">
                                <div class="product-img"> <img src="<?php echo $row['hinh1']?>" alt="" style="width: 252.5px; height: 252.5">
                                    <div class="product-body">
                                        ...
                                        <div class="product-btns">
                                            <a href = "admin_chiTietSP.php?product=<?= $row['id'] ?>" >
											    <button class="quick-view"><i
                                                    class="fa fa-pencil-square-o"></i>
                                            </a>
											</button>
											<a href="quanLySP.php?ctrlXoa=1&idSP=<?=$row['id']?>" class="quick-view"><i class="fa fa-trash-o"></i>
											</button>
										</div>
                                    </div>
                                </div>
                    ...
                    ?>
                        ...
                        <script type="text/javascript">
                            ...
                            //var count = <?php echo $page; $page = $page + 1; ?>;
                            ...
                            for (i = 1; i <= count; i++) {
                                if ( i == trang ){
                                    ...
                                    add += "<li><a name=li" + i + " href='admin_suaSP.php?<?php if (isset($_REQUEST['search'])) {echo "search="; echo $_REQUEST['search'];  } ?>&trang="+ i + "'>" + i + "</a></li>";
                                }
                            }
